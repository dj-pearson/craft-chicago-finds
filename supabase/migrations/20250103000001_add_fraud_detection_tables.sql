...
CREATE TABLE IF NOT EXISTS public.fraud_signals (
  ...
  order_id UUID, -- Will add foreign key constraint later if orders table exists
  ...
  action_taken TEXT,
  false_positive BOOLEAN DEFAULT NULL, -- NULL = unknown, true = false positive, false = true positive
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
BEGIN
  ...
  RETURN (
    user_trust < 30 OR                           -- Low trust score
    (transaction_amount > 500 AND user_trust < 50) OR  -- High amount + medium trust
    recent_transactions > 5 OR                   -- Too many recent transactions
    high_risk_signals > 0 OR                    -- Recent high-risk signals
    transaction_amount > 1000                    -- Very high amount
  );
END;
$$;
...
-- Trigger to update trust scores on order status changes (only if orders table exists)
DO $$
BEGIN
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
    -- Add foreign key constraint to fraud_signals.order_id
    IF NOT EXISTS (
    ...
    -- Add foreign key constraint to fraud_reviews.order_id
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints 
